...
namespace Sigtrap.VrTunnellingPro {
	...
	public abstract class TunnellingBase : MonoBehaviour {
		...
		public const float MOTION_STRENGTH_MAX = 4f;
		...
		private float _lastSpeed, _lastEffect=Mathf.Lerp(COVERAGE_MIN, 1, 0)*Mathf.Lerp(COVERAGE_MIN, 1, 0.744f);
        ...
        protected float EffectCoverageThreshold(float t)
        {
            
            float effectCoverageCurrent = 0.0000f;
            float a = (-RemapRadius(effectCoverage) + RemapRadius(0) * RemapRadius(effectCoverage)) / 100.0000f, b = RemapRadius(effectCoverage) - 80 * a;
            effectCoverageCurrent = a*t +b;
            return effectCoverageCurrent;
        }
        ...
        protected float CalculateMotion(float dT){
			...
			if (useAngularVelocity) {
                ...
                _avSmoothed = av;
                fx += Mathf.Abs(_avSmoothed * angularVelocityStrength/20);
            }
			...
			if (useVelocity) {
				...
				//}
				//fx += lm * velocityStrength * 4;
                fx += speed * velocityStrength * 4;
            }
            ...
            //if (fx == 0)
            //{
            //    if(_lastEffect < EffectCoverageThreshold(130))
            //    {
            //        _lastEffect = RemapRadius(0) * RemapRadius(effectCoverage);
            //        Debug.Log(">130");
            //    }
            //    else if(_lastEffect < EffectCoverageThreshold(80) && _lastEffect >= EffectCoverageThreshold(120))
            //    {
            //        fx = _lastEffect + RemapRadius(effectCoverage) * (1 - RemapRadius(0)) / 100 * 9;
            //        Debug.Log("<130, >80");
            //    }
            //    else
            //    {
            //        fx = _lastEffect + RemapRadius(effectCoverage) * (1 - RemapRadius(0)) / 100 * 3;
            //        fx = Mathf.Min(fx, RemapRadius(effectCoverage));
            //        Debug.Log("<80");
            //    }
            //    Debug.Log("FX" + _lastEffect);
            //}
            if (_lastEffect < EffectCoverageThreshold(130))
            {
                if(fx == 0)
                ...
                {
                    fx = _lastEffect + RemapRadius(effectCoverage) * (1 - RemapRadius(0)) / 100 * fx * 3;
					fx = Mathf.Max(0.7f, fx);
                }
            }
            else if(_lastEffect < EffectCoverageThreshold(80)&& _lastEffect >= EffectCoverageThreshold(130))
            {
                if(fx == 0)
                {
                    fx = _lastEffect - RemapRadius(effectCoverage) * (1 - RemapRadius(0)) / 100 *3;
                ...
                {
                    fx = _lastEffect + RemapRadius(effectCoverage) * (1 - RemapRadius(0)) / 100 * fx / 2;
                }
            }
            else
            {
                if(fx == 0)
                {
                    ...
                    fx = _lastEffect + RemapRadius(effectCoverage) * (1 - RemapRadius(0)) / 100 * fx;
                    fx = Mathf.Min(fx, RemapRadius(effectCoverage));
                }
            }
            _lastEffect = fx;
...
#endif
            //if (fx <= EffectCoverageThreshold(130) && !Mathf.Approximately(fx, RemapRadius(0) * RemapRadius(effectCoverage)))// Angle larger than 130
            //{
            //    fx += (-fx + _lastEffect) + 3*(fx - _lastEffect)*Time.deltaTime;
            //}
            //else if (fx <= EffectCoverageThreshold(80) && fx > EffectCoverageThreshold(120))// Angle between 80 and 120
            //{
            //    fx = _lastEffect+(fx-_lastEffect)/2*Time.deltaTime;
            //}
            //else
            //{
            //    fx = fx;
            //}
            //_lastEffect = Mathf.Clamp01(fx);
